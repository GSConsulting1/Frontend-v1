// Envoltorio de cliente de la pestaña "VoBo". Existe por el mismo motivo que
// clientes-listado.tsx: la acción en lote del header ("Eliminar personas",
// disparada desde el menú "⋮") necesita compartir con la tabla el Set de IDs
// seleccionados, así que ese estado se sube acá (lifting state up). page.tsx
// sigue siendo Server Component (hace el fetch) y solo renderiza este
// componente.
//
...
import { VoboTable } from "@/components/vobo/vobo-table";
import { VoboAccionesMenu } from "@/components/vobo/vobo-acciones-menu";
import { EliminarVoboButton } from "@/components/vobo/eliminar-vobo-button";
import { crearVobo } from "@/app/profesionales/vobo/actions";
import { voboSchema, type VoboFormValues } from "@/lib/validations/vobo.schema";
import type { Vobo } from "@/types";

type VoboListadoProps = {
  personas: Vobo[];
};

export function VoboListado({ personas }: VoboListadoProps) {
  ...
  async function onSubmit(values: VoboFormValues) {
    setServerError(null);
    const result = await crearVobo(values);
    ...
    <>
      ...
      <div className="relative w-full max-w-xs">
        ...
        <Input
          ...
          aria-label="Buscar persona de VoBo"
          className="pl-8"
        />
      </div>
      ...
      {formAbierto && (
        <form
          ...
          noValidate
        >
          <CamposVobo idPrefix="nuevo" register={register} errors={errors} />
      ...
      <VoboTable
        ...
        mensajeVacio={
          personas.length === 0
            ? "No hay personas de VoBo registradas."
