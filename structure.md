...
- "Valor hora profesional" vive en su propia tabla (`valor_hora_orden`,
  ...
  decisión de política que le toca a Persona A. El viejo
  `ordenes-manager.tsx` de "guardar cambios en lote" ya no existe (el
  listado pasó a solo lectura); el único Client Component que hoy envuelve
  el listado es `ordenes-listado.tsx`, y solo para compartir la selección
  de filas a exportar entre el header y la tabla (ver arriba).
